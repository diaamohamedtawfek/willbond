...
import 'package:willbond/trackorder/detales_item/item_draft.dart';
...
class UiTrackOrder extends State<DraftOrder>{
...
//      fetchData();
  ...
  Future getData(var search) async {
    ...
    print(URL_LOGIC.draft_orders_trackOrder + "${numpage}" + "&size=15");
    isLoading = true;
    setState(() {
      isLoading = true;
//      data_offer.clear();
...
//    };
    http.Response response_offer = await http.get(
        Uri.parse(URL_LOGIC.draft_orders_trackOrder + "${numpage}" + "&size=15"),
...
//                     //list item
                      ...
                      Padding(
                        ...
                        child: data_offer == null?HomeWiting()
                            :listItem_new(),
                      ),
                    ],
                  )),
...
//              ),
              ...
              itemBuilder: (_, index) {
                return InkWell(
                  onTap: ()=>
                      Navigator.push(context,MaterialPageRoute(builder: (context) => Item_Draft(id: "${_All?[index]["id"]??""}",)),),
                  child:
//                  Card(
